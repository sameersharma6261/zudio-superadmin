...
const Counter = () => {
  ...
  const handleClick = async (index) => {
    ...
    const userMobile = data[index].mobile;
    const selectedUser = data[index];

    try {
      // ✅ Step 1: Send message
      await handleSendMessage(userMobile, selectedUser);

      // ✅ Step 2: Start countdown
      await startCountdown(userMobile);

      // ✅ Step 3: Update status in the backend
      await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/api/update-status/${userId}`
      );

      // ✅ Step 4: Remove user from the UI
